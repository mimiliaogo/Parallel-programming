...
//color parallel version but slower?
//wrong toloerance
...
void* MandelbrotSet(void *arg_)
{
    ...
    //做data_num件事
    float wrong_cnt_float = (( data_all * 0.004) / NUM_THREAD);// wrong tolerance
    ...
    for (int t=*k; t<data_all; t+=NUM_THREAD) {
        ...
        //image[t] = repeats;
        /*directly calculate color */
        if (repeats != iters) {
            if (repeats & 16) {
                color_rgb[3*t] = 240;
                color_rgb[3*t+1] = color_rgb[3*t+2] = repeats % 16 * 16;
            } else {
                color_rgb[3*t] = repeats % 16 * 16;
            }
        }
    }
    pthread_exit(NULL);
...
int main(int argc, char** argv) {
    ...
    color_rgb = (int*) calloc(width * height * 3 , sizeof(int));
    ...
    /* draw and cleanup */
    write_png(filename, iters, width, height, color_rgb);
    free(color_rgb);
}